...
app.post(
  ...
  async (req, res) => {
    ...
    if (mailer.isConfigured()) {
      try { await mailer.sendPreinscription(data, photos); }
...
app.post('/api/admin/submissions/:id/validate', async (req, res) => {
  ...
  let mailed = false;
  if (mailer.isConfigured()) {
    try { await mailer.sendValidation(rec.fields); mailed = true; }
...
app.post('/api/admin/submissions/:id/reject', async (req, res) => {
  ...
  let mailed = false;
  if (mailer.isConfigured()) {
    try { await mailer.sendRejection(rec.fields); mailed = true; }
